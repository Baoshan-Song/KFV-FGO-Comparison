...
def motion(x: np.ndarray, dt: float, omega: float) -> np.ndarray:
    ...
    return np.array([x[0] + x[2] * dt, x[1] + x[3] * dt,
                     x[2] - omega * x[3] * dt, x[3] + omega * x[2] * dt])


def motion_jacobian(_x: np.ndarray, dt: float, omega: float) -> np.ndarray:
    return np.array([[1, 0, dt, 0], [0, 1, 0, dt],
                     [0, 0, 1, -omega * dt], [0, 0, omega * dt, 1]], dtype=float)
...
class FilterConfig:
    mode: str = "EKF"
    dt: float = 1.0
    err_x0: np.ndarray = field(default_factory=lambda: np.array([100., -100., 0., 0.]))
    p0: np.ndarray = field(default_factory=lambda: np.diag([50., 50., 1., 1.]))
...
def _config_to_dict(config: Config) -> dict:
    return {
        "data": {"mode": config.data_mode, "path": config.data_path},
        "KFV": {"mode": config.kfv.mode, "dt": config.kfv.dt,
                 "errX0": config.kfv.err_x0.tolist(), "P0": config.kfv.p0.tolist(),
                 "Q": config.kfv.q.tolist(), "omega": config.kfv.omega, "R": config.kfv.r,
                 "max_iteration": config.kfv.max_iteration,
                 "thres_iteration": config.kfv.threshold_iteration,
                 "robust_kernel": config.kfv.robust_kernel, "robust_delta": config.kfv.robust_delta,
                 "window_size": config.kfv.window_size},
        "FGO": {"dt": config.fgo.dt, "errX0": config.fgo.err_x0.tolist(),
                 "P0": config.fgo.p0.tolist(), "Q": config.fgo.q.tolist(),
                 "omega": config.fgo.omega, "R": config.fgo.r,
                 "max_iteration": config.fgo.max_iteration,
                 "thres_iteration": config.fgo.threshold_iteration,
                 "robust_kernel": config.fgo.robust_kernel, "robust_delta": config.fgo.robust_delta,
                 "window_size": config.fgo.window_size,
                 "imitate_KFV": config.fgo.imitate_kfv, "autoDiff": config.fgo.autodiff},
    }
...
def load_config(path: str | Path) -> Config:
    ...
    k = raw.get("KFV", {})
    f = raw.get("FGO", {})
    kfv = FilterConfig(mode=k.get("mode", "EKF"), dt=k.get("dt", 1.0),
                       ...
                       omega=k.get("omega", 2 * np.pi / 100), r=k.get("R", 0.01),
                       max_iteration=k.get("max_iteration", 2),
                       threshold_iteration=k.get("thres_iteration", 1e-6),
                       robust_kernel=k.get("robust_kernel", "huber"),
                       robust_delta=k.get("robust_delta", 2.0), window_size=k.get("window_size", 1))
    fgo = FgoConfig(dt=f.get("dt", kfv.dt),
                    err_x0=np.asarray(f.get("errX0", kfv.err_x0), dtype=float),
                    p0=np.asarray(f.get("P0", kfv.p0), dtype=float),
                    q=np.asarray(f.get("Q", kfv.q), dtype=float), omega=f.get("omega", kfv.omega),
                    r=f.get("R", kfv.r), max_iteration=f.get("max_iteration", 1),
                    threshold_iteration=f.get("thres_iteration", 1e-6),
                    robust_kernel=f.get("robust_kernel", "none"),
                    robust_delta=f.get("robust_delta", 2.0), window_size=f.get("window_size", 1),
                    imitate_kfv=f.get("imitate_KFV", False), autodiff=f.get("autoDiff", False))
    return Config(data_mode=data.get("mode", "sim"), data_path=data.get("path", "circle_cv_gmm_L4.mat"),
                  kfv=kfv, fgo=fgo)
...
def sw_fgo_config() -> Config:
    return Config(fgo=FgoConfig(imitate_kfv=False, autodiff=False, max_iteration=1,
                                 robust_kernel="none", window_size=1))


def convert_kfv_to_fgo(config: Config) -> Config:
    k = config.kfv
    f = FgoConfig(dt=k.dt, err_x0=k.err_x0.copy(), p0=k.p0.copy(), q=k.q.copy(), omega=k.omega,
                  r=k.r, max_iteration=k.max_iteration, threshold_iteration=k.threshold_iteration,
                  robust_kernel=k.robust_kernel, robust_delta=k.robust_delta,
                  window_size=k.window_size, imitate_kfv=True, autodiff=False)
    if k.mode == "EKF":
    ...
    return Config(data_mode=config.data_mode, data_path=config.data_path, kfv=k, fgo=f)
